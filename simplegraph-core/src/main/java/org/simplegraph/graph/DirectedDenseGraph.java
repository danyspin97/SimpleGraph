...
public class DirectedDenseGraph<V> extends BaseDenseGraph<V> implements DirectedGraph<V> {
    ...
    public boolean removeVertex(V vertex)
    {
        ...
        for (int i = 0; i != verticesCount; i++) {
            ...
            edges[verticesCount][i] = false;

            edges[i][oldVertex] = edges[i][verticesCount];
            edges[i][verticesCount] = false;
        }
    ...
    public LinkedList<V> getNeighbors(V vertex)
    {
        ...
        for (int i = 0; i != verticesCount; i++) {
            if (edges[vertexIndex][i] || edges[i][vertexIndex]) {
